...
def get_daily_metrics(raw_issues):
    ...
    for issue in raw_issues:
        ...
        issues[issue.key]['fields']['assignee'] = None if isinstance(issue.fields.assignee,
                                                                     type(None)) else issue.fields.assignee.displayName
        ...
        issues[issue.key]['t_current_status'] = get_time_in_current_status(issues[issue.key]['fields']['status'],
                                                                           issue.changelog)
        issues[issue.key]['t_overall'] = get_time_in_current_status(issues[issue.key]['fields']['status'],
                                                                    issue.changelog) if Status.IN_PROGRESS.value == issue.fields.status.name else get_time_between_distant_statuses(
            Status.IN_PROGRESS.value, issue.fields.status.name, issue.changelog)
        issues[issue.key]['pct_consumed'] = -1 if isinstance(issues[issue.key]['fields']['original_estimate'],
                                                             type(None)) else (issues[issue.key]['t_overall'][
                                                                                   'worktime'] /
                                                                               issues[issue.key]['fields'][
    ...
    for assignee, issues in group_issues_by_assignee(raw_issues).items():
        ...
        for issue in issues:
            if (assignee is None and not issue['fields']['status'].startswith("Ready")) \
                ...
                print("  * {} | {} | {}h".format(issue['key'], issue['fields']['status'],
                                                 seconds_to_hours(issue['t_current_status']['worktime'])))
            else:
                print("    {} | {} | {}h".format(issue['key'], issue['fields']['status'],
                                                 seconds_to_hours(issue['t_current_status']['worktime'])))
