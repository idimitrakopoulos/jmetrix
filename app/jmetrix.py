...
from util.toolkit import jira_token_authenticate, get_time_in_status, get_time_from_creation_to_extreme_status, \
    get_time_between_extreme_statuses, get_time_in_initial_status, ab_path_to_class, ab_subclass_path_from_action
...
if __name__ == '__main__':
    ...
    parser_live.add_argument('bar', type=int, help='bar help')
    ...
    ab = ab_path_to_class(ab_subclass_path_from_action(args.ab), parser)
    ...
    #     issues[issue.key]['fields']['size'] = issue.fields.timeoriginalestimate
    #
    ...
    #     issues[issue.key]['sizing_accuracy_pct'] = round((issues[issue.key]['fields']['size'] / issues[issue.key]['aggregates']['t_cycle']) * 100 if issues[issue.key]['fields']['size'] and issues[issue.key]['aggregates']['t_cycle'] else -1, 2)
    #
