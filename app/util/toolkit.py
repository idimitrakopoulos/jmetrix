...
def calc_working_seconds(from_date, to_date):
    ...
    workday_stop_time  = datetime.datetime.strptime("17:15:00", "%H:%M:%S").time()
        ...
        for item in history.items:
            if item.field == 'status':
                if item.toString == status and not in_flag:
                    ...
                    # log.debug("Found  " + item.toString + " on  " + history.created)
                    result.append(calc_working_seconds(in_time, out_time))
                    # log.debug("get_time_in_status {} = {} sec (working: {} actual: {})".format(status, result[len(result)-1], datetime.timedelta(seconds=result[len(result)-1]), str(out_time-in_time)))
